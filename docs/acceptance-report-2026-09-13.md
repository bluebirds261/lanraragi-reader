...
## 二·补十五 第十五轮：搜索面收口（按用户五条裁决）（D-27）

| # | 用户裁决 | 落地 |
|---|---|---|
| 1 | 以图搜图**不做** | 未实现（LANraragi 无对应端点），也不放装饰性入口 |
| 2 | 面板里的「排序与筛选」**去掉** | 展开面板只剩「收起 + 输入 + 清除」；排序/筛选仍在库页顶栏左侧键 |
| 3 | **结果内嵌**（EhViewer/JHenTai 那样） | 提交后展开层不关闭：面板固定在顶部，下方渲染结果列表（`SearchResultsGrid`，与库页共用同一份 `LibraryViewModel` 状态与 `ArchiveCard`，分页/长按语义一致），顶部显示「N 个结果」；编辑查询即退回「历史/联想」态（JHenTai 的 bodyType 切换） |
| 4 | 删除「标签浏览」页；历史与标签排布参考 JHenTai；热门标签单独一块；垃圾桶图标 | **删除 `ui/screens/SearchScreen.kt` 与 `Routes.SEARCH`**（含 composable 注册与常量）；展开层＝搜索历史（EHTag 胶囊 + 垃圾桶「管理历史」+ 隐藏 + 清空）→ 标签联想（两行：命名空间着色标题 + 词库译名，点选=包含/长按=排除）→ **独立「热门标签」区**（命名空间小胶囊 + 标签胶囊，来自服务端标签统计并排除日期类元数据） |
| 5 | 浅色模式下搜索面板可读性 | `liquidGlassCapsule()` 新增 `baseSurfaceAlpha`：浅色主题垫 0.86 的 surface（深色 0.42），scrim 也由 0.86 提到浅色 0.94 —— 纯玻璃在亮底上文字对比度不足 |

顺带修掉一个交互 bug：展开层原先把 scrim 的 `clickable` 套在整个面板外层，**把输入框的焦点与手势吃掉了**（真机上打不开键盘、输入无效）。
现改为 scrim 与内容并列的两层（scrim 在下、面板在上），输入框恢复正常。

真机验证：展开层含「搜索历史 / 管理历史（垃圾桶）/ 隐藏历史 / 清空」+ 热门标签独立区（命名空间胶囊：全部·source·艺术家·女性·团队…）；
面板内已无「排序与筛选」「标签浏览」；输入 `3` 回车 → 展开层内显示「30 个结果」并渲染结果卡片。
构建：`assembleDebug` + `testDebugUnitTest`（57 类 / 283 例 / 0 失败），编译告警 0。