...
> 2026-09-15 续：第一期（`0a9d92f`）已落库；第二轮针对用户真实库数据定位出的三个「看起来在工作、其实没生效」的问题做了修复（排序按服务器实际命名空间判真伪、联想候选本库优先并标注来源、跨屏总线重复定义），并回补了历史垃圾桶入口、联想键盘导航与排序/视图独立面板。详见实施记录的「第二轮」一节。本文件第 2 节的现状表与第 6 节的批次表仍是规划时口径，未逐条回填。
